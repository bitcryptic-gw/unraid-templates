# BitCryptic — Unraid Community Applications Templates

Docker template XML files for Unraid Community Applications.

## Templates

| Template | Description | Docker Hub | Forum |
|---|---|---|---|
| Paperclip | Open-source AI coding agent platform | [bitcryptic/paperclip](https://hub.docker.com/r/bitcryptic/paperclip) | [Support thread](https://forums.unraid.net/topic/198763-support-paperclip-open-source-ai-coding-agent-platform-for-unraid/) |
| NanoClaw | Self-hosted AI agent with Matrix E2EE and isolated containers | [bitcryptic/nanoclaw](https://hub.docker.com/r/bitcryptic/nanoclaw) | [Support thread](https://forums.unraid.net/topic/197773-support-nanoclaw-lightweight-secure-ai-agent-for-unraid/) |
| Pi Network Node | Mainnet Pi Network node, performance-tuned wrapper | [bitcryptic/pi-network-node](https://hub.docker.com/r/bitcryptic/pi-network-node) | [Support thread](https://forums.unraid.net/topic/197354-pi-network-node-docker-unraid-template-mainnet/) |
| ckpool-solo | Self-hosted Bitcoin solo mining stratum server | [bitcryptic/ckpool-solo](https://hub.docker.com/r/bitcryptic/ckpool-solo) | [Support thread](https://forums.unraid.net/topic/198377-support-ckpool-solo-self-hosted-bitcoin-solo-mining-for-unraid/) |

## Manual Installation

Add this repository to Unraid Community Applications:

`https://raw.github.com/bitcryptic-gw/unraid-templates`

Or drop any XML directly into `/boot/config/plugins/dockerMan/templates-user/` on your Unraid server.

## Links

- [BitCryptic](https://bitcryptic.com)
- [Docker Hub](https://hub.docker.com/u/bitcryptic)

---

<a href="https://buymeacoffee.com/bitcryptic">
  <img src="https://img.shields.io/badge/Buy%20Me%20A%20Coffee-Support%20my%20work-FFDD00?style=for-the-badge&logo=buy-me-a-coffee&logoColor=black">
</a>
